...
class Task3(Task):
  ...
  def spawn_task_objects(self):
    ...
    self._t3_run_id = str(int(time.time()))
    with open('/tmp/cw2_spawn_poses.json', 'w') as f:
      json.dump({'_run_id': self._t3_run_id}, f)
    ...
    # Signal that all spawning is complete
    try:
      with open('/tmp/cw2_spawn_poses.json', 'r') as f:
        data = json.load(f)
      data['_spawn_done'] = True
      with open('/tmp/cw2_spawn_poses.json', 'w') as f:
        json.dump(data, f)
    except Exception as e:
      self.node.get_logger().warn(f'Failed to write spawn_done: {e}')
