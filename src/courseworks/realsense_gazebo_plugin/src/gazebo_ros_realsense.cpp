...
void GazeboRosRealsense::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  // Use reliable QoS so downstream image/depth processing nodes (image_proc,
  // depth_image_proc) can subscribe without RELIABILITY mismatches.
  ...
  this->color_pub_ = image_transport::create_camera_publisher(
    ...
    cameraParamsMap_[COLOR_CAMERA_NAME].topic_name, reliable_qos);
  this->ir1_pub_ = image_transport::create_camera_publisher(
    this->node_.get(), prefix + std::string("/") +
    cameraParamsMap_[IRED1_CAMERA_NAME].topic_name, reliable_qos);
  this->ir2_pub_ = image_transport::create_camera_publisher(
    this->node_.get(), prefix + std::string("/") +
    cameraParamsMap_[IRED2_CAMERA_NAME].topic_name, reliable_qos);
  this->depth_pub_ = image_transport::create_camera_publisher(
    this->node_.get(), prefix + std::string("/") +
    cameraParamsMap_[DEPTH_CAMERA_NAME].topic_name, reliable_qos);
  ...
  const transport::PublisherPtr pub)
{
  ...
  // Preserve the latest color frame so IR callbacks do not overwrite the
  // image used for depth_registered pointcloud coloring.
  ...
  for (uint32_t j = 0; j < rows_arg; j++) {
      ...
      i++, ++iter_x, ++iter_y, ++iter_z, ++iter_rgb)
    {
      ...
      const bool has_color =
        this->image_msg2_.data.size() == rows_arg * cols_arg * 3;
      const bool has_mono =
        this->image_msg2_.data.size() == rows_arg * cols_arg;
      const uint8_t * image_src =
        (has_color || has_mono) ? this->image_msg2_.data.data() : nullptr;
      if (has_color) {
        // color
        if (this->image_msg2_.encoding == sensor_msgs::image_encodings::RGB8) {
          iter_rgb[2] = image_src[i * 3 + j * cols_arg * 3 + 0];
          iter_rgb[1] = image_src[i * 3 + j * cols_arg * 3 + 1];
          iter_rgb[0] = image_src[i * 3 + j * cols_arg * 3 + 2];
        } else if (this->image_msg2_.encoding == sensor_msgs::image_encodings::BGR8) {
